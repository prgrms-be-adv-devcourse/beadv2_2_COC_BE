...
public class RentalItem extends BaseEntity {
    ...
    public void markCanceled() {
        ...
        if (this.status == RentalItemStatus.RETURNED || this.status == RentalItemStatus.RENTING) {
            ...
            throw new IllegalStateException("이미 진행된 상품은 취소할 수 없습니다. rentalItemId: " + this.id + ", status: " + this.status);
        }
    ...
    public void cancelByRentalRequest() {
        ...
        if (!canCancelByRental()) {
            ...
            throw new IllegalStateException("진행 중이거나 완료된 상품은 취소할 수 없습니다. rentalItemId: " + this.id + ", status: " + this.status);
        }
    ...
    public void processReturn() {
        ...
        if (this.status != RentalItemStatus.RENTING && this.status != RentalItemStatus.ACCEPTED) {
            ...
            throw new IllegalStateException("현재 상태에서 반납 처리가 불가능합니다. rentalItemId: " + this.id + ", status: " + this.status);
        }
    ...
    public BigDecimal processRefund() {
        ...
        if (this.status != RentalItemStatus.ACCEPTED && this.status != RentalItemStatus.RETURNED) {
            ...
            throw new IllegalStateException("현재 상태에서 환불 처리가 불가능합니다. rentalItemId: " + this.id + ", status: " + this.status);
        }
    ...
    public BigDecimal extendRental(LocalDate newEndDate) {
        ...
        if (this.status == RentalItemStatus.CANCELED
            ...
            throw new IllegalStateException("취소/반납/거절된 상품은 연장할 수 없습니다. rentalItemId: " + this.id + ", status: " + this.status);
        }
