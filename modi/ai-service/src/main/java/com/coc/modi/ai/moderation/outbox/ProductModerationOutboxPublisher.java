...
public class ProductModerationOutboxPublisher {
	...
	public void publishPendingEvents() {
		...
		List<ProductModerationOutboxEvent> events = outboxEventRepository.findPendingForPublish(batchSize);

		for (ProductModerationOutboxEvent event : events) {
			try {
				publishEvent(event);
				event.markSent();
			} catch (Exception ex) {
	...
	private void publishEvent(ProductModerationOutboxEvent event) throws Exception {

		if (event.getEventType() == ProductModerationOutboxEventType.PRODUCT_MODERATION_RESULT) {
			ProductModerationResultEvent payload =
					readPayload(event.getPayload(), ProductModerationResultEvent.class);
			kafkaTemplate
